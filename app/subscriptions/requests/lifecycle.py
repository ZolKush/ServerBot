        ...
        if current_time >= end:
            ...
            if current.get("access_state") == "approved" and bool(current.get("enabled", True)):
                queue_message(
                    ...
                    text=(
                        "🤖 <b>Доступ понижен до базового</b>\n\n"
                        "Срок оплаченной подписки завершён, а оплата следующего периода "
                        "не подтверждена. Ваш уровень доступа в боте понижен до базового.\n\n"
                        "Вы можете создать тикет или оформить подписку. "
                        "Персональная ссылка сохранена. После подтверждения оплаты "
                        "доступ уровня подписчика будет восстановлен."
                    ),
                    reply_markup=[
                        [
                            {

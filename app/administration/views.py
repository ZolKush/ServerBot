...
def administration_markup(meta: dict[str, Any]) -> InlineKeyboardMarkup:
    ...
    if is_owner_meta(meta):
        rows.append([InlineKeyboardButton("📊 Выгрузить клиентов", callback_data="users:export:xlsx")])
...
def service_settings_text(settings: dict[str, Any], actor: dict[str, Any]) -> str:
    lines = ["⚙️ <b>Администрирование → Настройки сервиса</b>"]
    if is_owner_meta(actor):
        template = payment_template_from_settings(settings)
        preview = template if len(template) <= 700 else template[:700].rstrip() + "…"
        lines.extend(
            [
                "",
                ...
                "",
                "Подстановки: <code>{amount}</code>, <code>{months}</code>, <code>{access_until}</code>.",
                "",
...
def service_settings_markup(actor: dict[str, Any]) -> InlineKeyboardMarkup:
    ...
    if can_edit_help_meta(actor):
        rows.append([InlineKeyboardButton("📝 Изменить помощь", callback_data="administration:input:help")])
    if is_owner_meta(actor):
        rows.extend(
            [
                [

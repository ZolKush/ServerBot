...
На главном экране статуса показывается краткая сводка; подробности TLS и Docker открываются отдельными административными кнопками, а аварийные элементы выводятся сразу. Единая кнопка «Обновить» принудительно обновляет основные метрики, DNS и статусы Docker выбранного сервера, включая повторную проверку после ошибок Docker/SSH. Ошибка одной проверки не мешает обновлению остальных. Проверка disk/UFW через SSH появляется только при технической ошибке или неполном ответе основного RemnaWave monitoring и повторно проверяет это условие перед подключением.

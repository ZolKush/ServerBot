...
При миграции на схему v3 все существовавшие на тот момент записи `role=user` становятся подписчиками, чтобы обновление не отняло прежний функционал. Новые одобренные пользователи получают базовый доступ. Все существующие администраторы становятся специалистами поддержки. Схема v4 добавляет резервную почту, новые признаки напоминаний и нормализует платёжное состояние руководителя без ручного редактирования JSON.

Должности сотрудников: специалист поддержки, инженер сопровождения, ведущий инженер сопровождения и руководитель сервиса. Руководитель назначает должности. Каждый сотрудник может выбрать публичную подпись: только должность либо должность с псевдонимом. Реальное имя, username и Telegram ID остаются видны во внутренних административных представлениях. Административная роль не заменяет уровень VPN-доступа: все сотрудники оплачивают подписку на общих основаниях, кроме единственного руководителя сервиса, которому автоматически назначается бессрочный оплаченный доступ.
...
- указать в личном профиле резервную электронную почту для связи;
- получать объявления и уведомления о техработах.

Подписчик и пользователь с безлимитным тестовым доступом дополнительно видят состояние доступных серверов. Обычная оплаченная подписка автоматически предупреждает за три дня, за один день, за 15 минут и в момент окончания. Предварительные уведомления содержат настроенные реквизиты и кнопку сообщения об оплате, когда следующий период готов. После окончания уровень в боте становится базовым, а персональная ссылка и административная роль, если она есть, сохраняются.

Администратор дополнительно может:

- просматривать локальные и SSH-серверы: uptime, RAM, disk, DNS, UFW, полный счётчик состояний Docker и TLS-сертификаты настроенных доменов;
...
Инженер сопровождения, ведущий инженер и руководитель могут менять текст раздела «Помощь». Ведущий инженер сопровождения может вручную и массово менять дату окончания уже оплаченных подписок и отправлять напоминания. Подтверждать оплату, задавать реквизиты, глобальную почту администрации и платёжные периоды, назначать уровни и менять должности может только руководитель сервиса. Эти действия находятся в разделе «Администрирование».

Заблокированные учётные записи вынесены из общего списка пользователей в отдельный фильтр. После единственного итогового уведомления бот полностью игнорирует их команды, сообщения и старые callback-кнопки; ожидавшие доставки сообщения также подавляются.

Глобальная почта администрации выводится в разделах «Помощь» и «Тикеты» как резервный канал связи. Это отдельная настройка сервиса, а не резервная почта конкретного пользователя.
...
`app/requirements.txt` содержит проверенный runtime lock: зафиксированы и прямые, и совместимые transitive-зависимости. `idna`, `cryptography` и актуальный CA bundle закреплены явно для HTTP/TLS-стека и проверки сертификатов. Метаданные `pyproject.toml` перечисляют только прямые зависимости; после любого изменения lock весь набор проверяется заново.
...
- напоминания об активных техработах каждые 30 минут;
- очистка rate-limit авторизации;
- освобождение тикетов, оставшихся назначенными несуществующему администратору;
- обработка платёжного периода, освобождение зависших заявок, напоминания T−3/T−1/T−15 минут и завершение оплаченного доступа;
- startup-обновление Docker-кэша и повторная проверка каждые шесть часов; обычное открытие статуса не выполняет Docker-команды;
- startup-проверка TLS-сертификатов настроенных доменов и повторная проверка каждые шесть часов с одноразовыми оповещениями администраторам за три дня и после истечения;
- startup-проверка и периодическое удаление старых сообщений личных чатов с сохранением последнего сообщения.

DNS-проверки выполняются конкурентно с глобальным пределом, а SSH/status-запросы к одному серверу дедуплицируются и кратко кэшируются. Состояние Docker читается только из отдельного постоянного шестичасового кэша; живые Docker-команды остаются лишь в явных административных действиях диагностики, inspect и logs. Subprocess и HTTP-ответы имеют таймауты и жёсткие лимиты размера.
...
Если в данных ещё нет руководителя сервиса, после запуска войдите своим обычным администраторским паролем и один раз выполните `/owner <OWNER_PASSWORD>`. Затем в разделе «Администрирование» настройте реквизиты, глобальную почту администрации, текущую и следующую даты платёжного периода. При этом обновлении обязательно повторно выполните установку `app/requirements.txt`: проверка TLS использует добавленную зависимость `cryptography`.

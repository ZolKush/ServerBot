...
Подписчики дополнительно видят доступное им состояние серверов. Для оплаченных подписок предусмотрены напоминания за три дня, один день и 15 минут. Если срок закончился, а оплата следующего периода не подтверждена, доступ в боте понижается до базового с личным уведомлением и кнопками покупки подписки и обращения в поддержку. Уведомление сохраняется вместе с изменением доступа в очереди доставки; при подтверждении продления до его отправки устаревшее уведомление отменяется.
...
указанный в `.python-version`. Совместимость с Python 3.10 пока сохранена, но отдельно в CI не проверяется;
...
На главном экране статуса показывается краткая сводка; подробности TLS и Docker открываются отдельными административными кнопками, а аварийные элементы выводятся сразу. Единая кнопка «Обновить» принудительно обновляет основные метрики, DNS, статусы Docker и выполняет сетевую проверку TLS-сертификатов выбранного сервера, включая повторную проверку после ошибок Docker/SSH/TLS. Сохранённые сертификаты остальных серверов не удаляются; общий TLS endpoint проверяется один раз с сохранением всех привязок. Ошибка одной проверки не мешает обновлению остальных. Проверка disk/UFW через SSH появляется только при технической ошибке или неполном ответе основного RemnaWave monitoring и повторно проверяет это условие перед подключением.
...
GitHub Actions выполняет обязательные проверки на Python 3.14.7. Локально используйте SDK проекта:

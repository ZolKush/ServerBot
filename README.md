...
- управлять доступом, пользователями и персональными ссылками, просматривать ссылку из карточки пользователя;
- одобрять стандартный тест на 24 часа, отклонять заявки и отправлять реквизиты с индивидуальными ценой и сроком;
...
Руководитель меняет стандартную стоимость в разделе «Администрирование → Настройки сервиса → Изменить стандартную цену».
Это сумма за стандартный период в три месяца: по умолчанию 300 ₽, допустимы целые суммы от 1 до 1 000 000 ₽.
Новая цена применяется к новым заявкам и напоминаниям следующего периода. Созданная заявка сохраняет свою сумму.

До отправки реквизитов любой администратор может нажать «Цена и срок» в заявке на покупку.
Ввод `500 | 2` задаёт 500 ₽ за два календарных месяца от момента ввода; последний день короткого месяца учитывается автоматически.
Формат `500 | 2 | 31.12.2026 23:59` позволяет явно указать дату окончания в часовом поясе бота.
Срок — от 1 до 36 месяцев, дата — строго в будущем. Перед сохранением бот показывает сумму, срок и точную дату.
Затем администратор отправляет реквизиты из обновлённой карточки. Одновременное изменение другим сотрудником
или отправка реквизитов отменяют устаревшее подтверждение. После отправки изменить эти условия нельзя.

Индивидуальные условия относятся только к одной оплате и сохраняются в заявке, без постоянного тарифа в профиле пользователя.
После подтверждения оплаты пользователю назначается указанная дата окончания; следующее продление использует
актуальную стандартную цену и общий платёжный период. Подтверждать поступление оплаты по-прежнему может только руководитель.
Изменение срока в MaintBot не меняет срок доступа во внешней панели RemnaWave.

В тексте реквизитов используйте `{amount}`, `{months}`, `{access_until}` для суммы, числа месяцев и даты окончания.
Если подстановок нет, бот добавляет актуальные условия отдельным блоком перед реквизитами;
индивидуальные условия всегда выделяются таким блоком. Ранее введённые вручную суммы в самом тексте реквизитов
нужно заменить подстановками, чтобы они не противоречили текущей цене.

Кнопка «Посмотреть ссылку» в административной карточке показывает сохранённую ссылку самому сотруднику;
пользователю уведомление не отправляется. Длинная ссылка передаётся полностью отдельным файлом.
Рассылка только сотрудникам помечается «Массовая рассылка только администраторам» в доставленном сообщении и подтверждении отправителя.

Для этих настроек отдельная миграция split-layout не требуется: прежние данные получают стандартную цену по умолчанию.
Изменения заявок, настроек и аудита проходят через существующие транзакции хранилища.
